...
//it wont attach the bearer token to the request if it is the client side rendered
...
{
    headers:()=>({Authorization: isClient ? `Bearer ${window.localStorage.getItem("__twitter_token")}` : ""})
})